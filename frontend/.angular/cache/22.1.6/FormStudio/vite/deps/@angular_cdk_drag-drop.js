import { Bi as signal, D as DestroyRef, I as EventEmitter, Mn as forwardRef, P as EnvironmentInjector, T as DOCUMENT, U as InjectionToken, W as Injector, _ as untracked, et as NgZone, rt as PLATFORM_ID, sa as ɵɵdefineInjector, xr as inject, y as APP_ID, yn as effect } from "./_resource-chunk-DsXUkD4F.js";
import { Ai as setClassMetadata, Ga as ɵɵdefineService, Gn as RendererFactory2, Go as ɵɵprojectionDef, Ha as ɵɵdefineDirective, Io as ɵɵloadQuery, Ji as ɵɵInheritDefinitionFeature, Jt as Component, M as createComponent, Nn as NgModule, O as booleanAttribute, Qa as ɵɵdomElementStart, S as ViewChild, Sn as Input, Ts as ɵɵstyleProp, Ua as ɵɵdefineNgModule, Va as ɵɵdefineComponent, Vt as ApplicationRef, Wn as Renderer2, Wo as ɵɵprojection, Xi as ɵɵProvidersFeature, Yi as ɵɵNgOnChangesFeature, Za as ɵɵdomElementEnd, Zi as ɵɵadvance, Zn as Service, aa as ɵɵattribute, fn as ElementRef, fr as ViewContainerRef, ir as TemplateRef, is as ɵɵqueryRefresh, mo as ɵɵgetInheritedFactory, p as IterableDiffers, pr as ViewEncapsulation, qa as ɵɵdomElement, qs as ɵɵviewQuery, r as ChangeDetectorRef, rt as numberAttribute, sa as ɵɵclassProp, un as Directive, vr as afterNextRender, zn as Output } from "./core-O5ilEZO9.js";
import { Ct as take, D as shareReplay, Dn as isObservable, Qn as Subject, U as pairwise, Ut as auditTime, Vn as animationFrameScheduler, Xt as filter, Zn as BehaviorSubject, b as switchMap, g as takeUntil, gt as distinctUntilChanged, jn as of, m as tap, nn as interval, qn as asapScheduler, rr as Observable, tn as merge, tr as ConnectableObservable, ur as Subscription, vn as map, x as startWith } from "./esm5-P1D0zK7h.js";
import { u as isPlatformBrowser } from "./common-D0Hf1u5Q.js";
...
//#endregion
//#region ../node_modules/@angular/cdk/fesm2022/drag-drop.mjs
function deepCloneNode(node) {
	const clone = node.cloneNode(true);
	const descendantsWithId = clone.querySelectorAll("[id]");
	const nodeName = node.nodeName.toLowerCase();
	clone.removeAttribute("id");
	for (let i = 0; i < descendantsWithId.length; i++) descendantsWithId[i].removeAttribute("id");
	if (nodeName === "canvas") transferCanvasData(node, clone);
	else if (nodeName === "input" || nodeName === "select" || nodeName === "textarea") transferInputData(node, clone);
...
function transferData(selector, node, clone, callback) {
	...
	if (descendantElements.length) {
		...
		for (let i = 0; i < descendantElements.length; i++) callback(descendantElements[i], cloneElements[i]);
	}
}
var cloneUniqueId = 0;
function transferInputData(source, clone) {
	if (clone.type !== "file") clone.value = source.value;
	if (clone.type === "radio" && clone.name) clone.name = `mat-clone-${clone.name}-${cloneUniqueId++}`;
}
...
var ParentPositionTracker = class {
	...
	handleScroll(event) {
		...
		this.positions.forEach((position, node) => {
			if (position.clientRect && target !== node && target.contains(node)) adjustDomRect(position.clientRect, topDifference, leftDifference);
		});
...
function getRootNode(viewRef, _document) {
	...
	if (rootNodes.length === 1 && rootNodes[0].nodeType === _document.ELEMENT_NODE) return rootNodes[0];
...
function extendStyles(dest, source, importantProperties) {
	for (let key in source) if (source.hasOwnProperty(key)) {
		...
		if (value) dest.setProperty(key, value, importantProperties?.has(key) ? "important" : "");
		else dest.removeProperty(key);
	}
	return dest;
}
...
var capturingEventOptions = { capture: true };
var activeCapturingEventOptions$1 = {
	passive: false,
	capture: true
};
var _ResetsLoader = class _ResetsLoader {
...
var DragDropRegistry = class DragDropRegistry {
	_ngZone = inject(NgZone);
	_document = inject(DOCUMENT);
	...
	registerDropContainer(drop) {
		if (!this._dropInstances.has(drop)) this._dropInstances.add(drop);
	}
	registerDragItem(drag) {
		this._dragInstances.add(drag);
		if (this._dragInstances.size === 1) this._ngZone.runOutsideAngular(() => {
	...
	removeDragItem(drag) {
		...
		if (this._dragInstances.size === 0) this._cleanupDocumentTouchmove?.();
	}
	startDragging(drag, event) {
		if (this._activeDragInstances().indexOf(drag) > -1) return;
	...
	scrolled(shadowRoot) {
		...
		if (shadowRoot && shadowRoot !== this._document) streams.push(new Observable((observer) => {
			return this._ngZone.runOutsideAngular(() => {
				const cleanup = this._renderer.listen(shadowRoot, "scroll", (event) => {
					if (this._activeDragInstances().length) observer.next(event);
				}, capturingEventOptions);
				return () => {
					cleanup();
				};
	...
	_preventDefaultWhileDragging = (event) => {
		if (this._activeDragInstances().length > 0) event.preventDefault();
	};
	_persistentTouchmoveListener = (event) => {
		if (this._activeDragInstances().length > 0) {
			if (this._activeDragInstances().some(this._draggingPredicate)) event.preventDefault();
...
(() => {
	(typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DragDropRegistry, [{ type: Service }], null, null);
})();
...
var PreviewRef = class {
	...
	attach(parent) {
		...
		if (supportsPopover(this._preview)) this._preview["showPopover"]();
	}
	destroy() {
	...
	_createPreview() {
		...
		if (previewTemplate && previewConfig) {
			...
			if (previewConfig.matchSize) matchElementSize(preview, rootRect);
			else preview.style.transform = getTransform(this._pickupPositionOnPage.x, this._pickupPositionOnPage.y);
		} else {
			preview = deepCloneNode(this._rootElement);
			matchElementSize(preview, this._initialDomRect);
			if (this._initialTransform) preview.style.transform = this._initialTransform;
		}
		...
		if (previewClass) if (Array.isArray(previewClass)) previewClass.forEach((className) => preview.classList.add(className));
		else preview.classList.add(previewClass);
		return preview;
	}
};
...
var activeEventListenerOptions = { passive: false };
...
var DragRef = class {
	...
	withHandles(handles) {
		...
		this._disabledHandles.forEach((handle) => {
			if (this._handles.indexOf(handle) > -1) disabledHandles.add(handle);
		});
	...
	withRootElement(rootElement) {
		...
		if (typeof SVGElement !== "undefined" && this._rootElement instanceof SVGElement) this._ownerSVGElement = this._rootElement.ownerSVGElement;
		return this;
	}
	withBoundaryElement(boundaryElement) {
		this._boundaryElement = boundaryElement ? coerceElement(boundaryElement) : null;
		this._resizeSubscription.unsubscribe();
		if (boundaryElement) this._resizeSubscription = this._viewportRuler.change(10).subscribe(() => this._containInsideBoundaryOnResize());
		return this;
	}
	withParent(parent) {
	...
	dispose() {
		this._removeRootElementListeners();
		if (this.isDragging()) this._rootElement?.remove();
		this._marker?.remove();
	...
	resetToBoundary() {
		if (this._boundaryElement && this._rootElement && isOverflowingParent(this._boundaryElement.getBoundingClientRect(), this._rootElement.getBoundingClientRect())) {
			...
			let offsetX = 0;
			let offsetY = 0;
			if (childRect.left < parentRect.left) offsetX = parentRect.left - childRect.left;
			else if (childRect.right > parentRect.right) offsetX = parentRect.right - childRect.right;
			if (childRect.top < parentRect.top) offsetY = parentRect.top - childRect.top;
			else if (childRect.bottom > parentRect.bottom) offsetY = parentRect.bottom - childRect.bottom;
	...
	setFreeDragPosition(value) {
		...
		if (!this._dropContainer) this._applyRootElementTransform(value.x, value.y);
		return this;
	}
	...
	_sortFromLastPointerPosition() {
		...
		if (position && this._dropContainer) this._updateActiveDropContainer(this._getConstrainedPointerPosition(position), position);
	}
	_removeListeners() {
	...
	_pointerDown = (event) => {
		...
		if (this._handles.length) {
			...
			if (targetHandle && !this._disabledHandles.has(targetHandle) && !this.disabled) this._initializeDragSequence(targetHandle, event);
		} else if (!this.disabled) this._initializeDragSequence(this._rootElement, event);
	};
	_pointerMove = (event) => {
		const pointerPosition = this._getPointerPositionOnPage(event);
		if (!this._hasStartedDragging()) {
			if (Math.abs(pointerPosition.x - this._pickupPositionOnPage.x) + Math.abs(pointerPosition.y - this._pickupPositionOnPage.y) >= this._config.dragStartThreshold) {
		...
		if (this._dropContainer) this._updateActiveDropContainer(constrainedPointerPosition, pointerPosition);
		else {
		...
		if (this._moveEvents.observers.length) this._ngZone.run(() => {
	...
	_endDragSequence(event) {
		if (!this._dragDropRegistry.isDragging(this)) return;
		this._removeListeners();
		this._dragDropRegistry.stopDragging(this);
		this._toggleNativeDragInteractions();
		if (this._handles) this._rootElement.style.webkitTapHighlightColor = this._rootElementTapHighlight;
		if (!this._hasStartedDragging()) return;
		this.released.next({
			source: this,
			event
		});
	...
	_startDragSequence(event) {
		if (isTouchEvent(event)) this._lastTouchEventTime = Date.now();
		this._toggleNativeDragInteractions();
		const shadowRoot = this._getShadowRoot();
		const dropContainer = this._dropContainer;
		if (shadowRoot) this._ngZone.runOutsideAngular(() => {
	...
	_initializeDragSequence(referenceElement, event) {
		if (this._parentDragRef) event.stopPropagation();
		...
		if (target && target.draggable && event.type === "mousedown") event.preventDefault();
		if (isDragging || isAuxiliaryMouseButton || isSyntheticEvent || isFakeEvent) return;
		if (this._handles.length) {
		...
		if (this._boundaryElement) this._boundaryRect = getMutableClientRect(this._boundaryElement);
	...
	_updateActiveDropContainer({ x, y }, { x: rawX, y: rawY }) {
		...
		if (!newContainer && this._dropContainer !== this._initialContainer && this._initialContainer._isOverContainer(x, y)) newContainer = this._initialContainer;
		if (newContainer && newContainer !== this._dropContainer) this._ngZone.run(() => {
		...
		if (this.isDragging()) {
			...
			if (this.constrainPosition) this._applyPreviewTransform(x, y);
			else this._applyPreviewTransform(x - this._pickupPositionInElement.x, y - this._pickupPositionInElement.y);
		}
	}
	...
	_createPlaceholderElement() {
		...
		} else placeholder = deepCloneNode(this._rootElement);
	...
	_getConstrainedPointerPosition(point) {
		...
		if (this.lockAxis === "x" || dropContainerLock === "x") y = this._pickupPositionOnPage.y - (this.constrainPosition ? this._pickupPositionInElement.y : 0);
		else if (this.lockAxis === "y" || dropContainerLock === "y") x = this._pickupPositionOnPage.x - (this.constrainPosition ? this._pickupPositionInElement.x : 0);
	...
	_applyRootElementTransform(x, y) {
		...
		if (this._initialTransform == null) this._initialTransform = styles.transform && styles.transform != "none" ? styles.transform : "";
	...
	_containInsideBoundaryOnResize() {
		...
		if (x === 0 && y === 0 || this.isDragging() || !this._boundaryElement) return;
		const elementRect = this._rootElement.getBoundingClientRect();
		const boundaryRect = this._boundaryElement.getBoundingClientRect();
		if (boundaryRect.width === 0 && boundaryRect.height === 0 || elementRect.width === 0 && elementRect.height === 0) return;
		...
		if (x !== this._passiveTransform.x || y !== this._passiveTransform.y) this.setFreeDragPosition({
			y,
			x
		});
	}
	_getDragStartDelay(event) {
		...
		else if (isTouchEvent(event)) return value.touch;
	...
	_updateOnScroll(event) {
		...
		if (scrollDifference) {
			const target = _getEventTarget(event);
			if (this._boundaryRect && target !== this._boundaryElement && target.contains(this._boundaryElement)) adjustDomRect(this._boundaryRect, scrollDifference.top, scrollDifference.left);
	...
	_getShadowRoot() {
		if (this._cachedShadowRoot === void 0) this._cachedShadowRoot = _getShadowRoot(this._rootElement);
		return this._cachedShadowRoot;
	}
	_getPreviewInsertionPoint(initialParent, shadowRoot) {
		const previewContainer = this._previewContainer || "global";
		if (previewContainer === "parent") return initialParent;
	...
	_getPreviewRect() {
		if (!this._previewRect || !this._previewRect.width && !this._previewRect.height) this._previewRect = this._preview ? this._preview.getBoundingClientRect() : this._initialDomRect;
		return this._previewRect;
	}
	_nativeDragStart = (event) => {
		if (this._handles.length) {
			const targetHandle = this._getTargetHandle(event);
			if (targetHandle && !this._disabledHandles.has(targetHandle) && !this.disabled) event.preventDefault();
		} else if (!this.disabled) event.preventDefault();
	};
	...
	_conditionallyInsertAnchor(newContainer, exitContainer, nextItemElement) {
		if (newContainer === this._initialContainer) {
			...
			if (nextItemElement) nextItemElement.before(anchor);
			else coerceElement(exitContainer.element).appendChild(anchor);
		}
	}
};
...
function transferArrayItem(currentArray, targetArray, currentIndex, targetIndex) {
	...
	if (currentArray.length) targetArray.splice(to, 0, currentArray.splice(from, 1)[0]);
}
function copyArrayItem(currentArray, targetArray, currentIndex, targetIndex) {
	const to = clamp(targetIndex, targetArray.length);
	if (currentArray.length) targetArray.splice(to, 0, currentArray[currentIndex]);
}
...
var SingleAxisSortStrategy = class {
	...
	sort(item, pointerX, pointerY, pointerDelta) {
		...
		if (newIndex === -1 && siblings.length > 0) return null;
		const isHorizontal = this.orientation === "horizontal";
	...
	enter(item, pointerX, pointerY, index) {
		...
		if (currentIndex > -1) activeDraggables.splice(currentIndex, 1);
		const newIndex = index == null || index < 0 ? this._getItemIndexFromPointerPosition(item, pointerX, pointerY) : index;
		let newPositionReference = activeDraggables[newIndex];
		if (newPositionReference === item) newPositionReference = activeDraggables[newIndex + 1];
		if (!newPositionReference && (newIndex == null || newIndex === -1 || newIndex < activeDraggables.length - 1) && this._shouldEnterAsFirstChild(pointerX, pointerY)) newPositionReference = activeDraggables[0];
	...
	updateOnScroll(topDifference, leftDifference) {
		...
		this._itemPositions.forEach(({ drag }) => {
			if (this._dragDropRegistry.isDragging(drag)) drag._sortFromLastPointerPosition();
		});
	}
	...
	_getItemOffsetPx(currentPosition, newPosition, delta) {
		...
		if (delta === -1) itemOffset += isHorizontal ? newPosition.width - currentPosition.width : newPosition.height - currentPosition.height;
		return itemOffset;
	}
	_getSiblingOffsetPx(currentIndex, siblings, delta) {
		...
		if (immediateSibling) {
			...
			if (delta === -1) siblingOffset -= immediateSibling.clientRect[start] - currentPosition[end];
			else siblingOffset += currentPosition[start] - immediateSibling.clientRect[end];
		}
	...
	_getItemIndexFromPointerPosition(item, pointerX, pointerY, delta) {
		...
		const index = this._itemPositions.findIndex(({ drag, clientRect }) => {
			...
			if (delta) {
				...
				if (drag === this._previousSwap.drag && this._previousSwap.overlaps && direction === this._previousSwap.delta) return false;
			}
...
var MixedSortStrategy = class {
	...
	sort(item, pointerX, pointerY, pointerDelta) {
		...
		if (newIndex === -1 || this._activeItems[newIndex] === item) return null;
		const toSwapWith = this._activeItems[newIndex];
		if (previousSwap.drag === toSwapWith && previousSwap.overlaps && previousSwap.deltaX === pointerDelta.x && previousSwap.deltaY === pointerDelta.y) return null;
		const previousIndex = this.getItemIndex(item);
		const current = item.getPlaceholderElement();
		const overlapElement = toSwapWith.getRootElement();
		if (newIndex > previousIndex) overlapElement.after(current);
		else overlapElement.before(current);
	...
	enter(item, pointerX, pointerY, index) {
		...
		if (currentIndex > -1) this._activeItems.splice(currentIndex, 1);
		let enterIndex = index == null || index < 0 ? this._getItemIndexFromPointerPosition(item, pointerX, pointerY) : index;
		if (enterIndex === -1) enterIndex = this._getClosestItemIndexToPointer(item, pointerX, pointerY);
	...
	reset() {
		...
		for (let i = this._relatedNodes.length - 1; i > -1; i--) {
			...
			if (node.parentNode === root && node.nextSibling !== nextSibling) {
				if (nextSibling === null) root.appendChild(node);
				else if (nextSibling.parentNode === root) root.insertBefore(node, nextSibling);
			}
		}
		this._relatedNodes = [];
		this._activeItems = [];
	...
	updateOnScroll() {
		this._activeItems.forEach((item) => {
			if (this._dragDropRegistry.isDragging(item)) item._sortFromLastPointerPosition();
		});
	}
	withElementContainer(container) {
	...
	_getRootNode() {
		if (!this._rootNode) this._rootNode = _getShadowRoot(this._element) || this._document;
...
var DROP_PROXIMITY_THRESHOLD = .05;
var SCROLL_PROXIMITY_THRESHOLD = .05;
...
var DropListRef = class {
	...
	enter(item, pointerX, pointerY, index) {
		this._draggingStarted();
		if (index == null && this.sortingDisabled) index = this._draggables.indexOf(item);
	...
	withItems(items) {
		...
		if (this.isDragging()) if (previousItems.filter((item) => item.isDragging()).every((item) => items.indexOf(item) === -1)) this._reset();
		else this._sortStrategy.withItems(this._draggables);
		return this;
	}
	withDirection(direction) {
		this._direction = direction;
		if (this._sortStrategy instanceof SingleAxisSortStrategy) this._sortStrategy.direction = direction;
		return this;
	}
	...
	withOrientation(orientation) {
		if (orientation === "mixed") this._sortStrategy = new MixedSortStrategy(this._document, this._dragDropRegistry);
		else {
	...
	withElementContainer(container) {
		if (container === this._container) return this;
		const element = coerceElement(this.element);
		if ((typeof ngDevMode === "undefined" || ngDevMode) && container !== element && !element.contains(container)) throw new Error("Invalid DOM structure for drop list. Alternate container element must be a descendant of the drop list.");
		const oldContainerIndex = this._scrollableElements.indexOf(this._container);
		const newContainerIndex = this._scrollableElements.indexOf(container);
		if (oldContainerIndex > -1) this._scrollableElements.splice(oldContainerIndex, 1);
		if (newContainerIndex > -1) this._scrollableElements.splice(newContainerIndex, 1);
		if (this._sortStrategy) this._sortStrategy.withElementContainer(container);
	...
	_sortItem(item, pointerX, pointerY, pointerDelta) {
		if (this.sortingDisabled || !this._domRect || !isPointerNearDomRect(this._domRect, DROP_PROXIMITY_THRESHOLD, pointerX, pointerY)) return;
		const result = this._sortStrategy.sort(item, pointerX, pointerY, pointerDelta);
		if (result) this.sorted.next({
	...
	_startScrollingIfNecessary(pointerX, pointerY) {
		...
		this._parentPositions.positions.forEach((position, element) => {
			if (element === this._document || !position.clientRect || scrollNode) return;
			if (isPointerNearDomRect(position.clientRect, DROP_PROXIMITY_THRESHOLD, pointerX, pointerY)) {
				[verticalScrollDirection, horizontalScrollDirection] = getElementScrollDirections(element, position.clientRect, this._direction, pointerX, pointerY);
				if (verticalScrollDirection || horizontalScrollDirection) scrollNode = element;
			}
		});
		...
		if (scrollNode && (verticalScrollDirection !== this._verticalScrollDirection || horizontalScrollDirection !== this._horizontalScrollDirection || scrollNode !== this._scrollNode)) {
			...
			if ((verticalScrollDirection || horizontalScrollDirection) && scrollNode) this._ngZone.runOutsideAngular(this._startScrollInterval);
	...
	_draggingStarted() {
		...
		if ((typeof ngDevMode === "undefined" || ngDevMode) && this._container !== coerceElement(this.element)) {
			for (const drag of this._draggables) if (!drag.isDragging() && drag.getVisibleElement().parentNode !== this._container) throw new Error("Invalid DOM structure for drop list. All items must be placed directly inside of the element container.");
		}
	...
	_startScrollInterval = () => {
		...
		interval(0, animationFrameScheduler).pipe(takeUntil(this._stopScrollTimers)).subscribe(() => {
			...
			if (this._verticalScrollDirection === AutoScrollVerticalDirection.UP) node.scrollBy(0, -scrollStep);
			else if (this._verticalScrollDirection === AutoScrollVerticalDirection.DOWN) node.scrollBy(0, scrollStep);
			if (this._horizontalScrollDirection === AutoScrollHorizontalDirection.LEFT) node.scrollBy(-scrollStep, 0);
			else if (this._horizontalScrollDirection === AutoScrollHorizontalDirection.RIGHT) node.scrollBy(scrollStep, 0);
		});
	};
	...
	_canReceive(item, x, y) {
		if (!this._domRect || !isInsideClientRect(this._domRect, x, y) || !this.enterPredicate(item, this)) return false;
	...
	_listenToScrollEvents() {
		this._viewportScrollSubscription = this._dragDropRegistry.scrolled(this._getShadowRoot()).subscribe((event) => {
			if (this.isDragging()) {
				const scrollDifference = this._parentPositions.handleScroll(event);
				if (scrollDifference) this._sortStrategy.updateOnScroll(scrollDifference.top, scrollDifference.left);
			} else if (this.isReceiving()) this._cacheParentPositions();
		});
	}
	_getShadowRoot() {
...
function getVerticalScrollDirection(clientRect, pointerY) {
	...
	if (pointerY >= top - yThreshold && pointerY <= top + yThreshold) return AutoScrollVerticalDirection.UP;
	else if (pointerY >= bottom - yThreshold && pointerY <= bottom + yThreshold) return AutoScrollVerticalDirection.DOWN;
	return AutoScrollVerticalDirection.NONE;
}
function getHorizontalScrollDirection(clientRect, pointerX) {
	const { left, right, width } = clientRect;
	const xThreshold = width * SCROLL_PROXIMITY_THRESHOLD;
	if (pointerX >= left - xThreshold && pointerX <= left + xThreshold) return AutoScrollHorizontalDirection.LEFT;
	else if (pointerX >= right - xThreshold && pointerX <= right + xThreshold) return AutoScrollHorizontalDirection.RIGHT;
...
function getElementScrollDirections(element, clientRect, direction, pointerX, pointerY) {
	...
	if (computedVertical) {
		...
		if (computedVertical === AutoScrollVerticalDirection.UP) {
			if (scrollTop > 0) verticalScrollDirection = AutoScrollVerticalDirection.UP;
		} else if (element.scrollHeight - scrollTop > element.clientHeight) verticalScrollDirection = AutoScrollVerticalDirection.DOWN;
	}
	if (computedHorizontal) {
		const scrollLeft = element.scrollLeft;
		if (direction === "rtl") {
			if (computedHorizontal === AutoScrollHorizontalDirection.RIGHT) {
				if (scrollLeft < 0) horizontalScrollDirection = AutoScrollHorizontalDirection.RIGHT;
			} else if (element.scrollWidth + scrollLeft > element.clientWidth) horizontalScrollDirection = AutoScrollHorizontalDirection.LEFT;
		} else if (computedHorizontal === AutoScrollHorizontalDirection.LEFT) {
			if (scrollLeft > 0) horizontalScrollDirection = AutoScrollHorizontalDirection.LEFT;
		} else if (element.scrollWidth - scrollLeft > element.clientWidth) horizontalScrollDirection = AutoScrollHorizontalDirection.RIGHT;
	}
	return [verticalScrollDirection, horizontalScrollDirection];
}
var DragDrop = class DragDrop {
	_injector = inject(Injector);
...
(() => {
	(typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DragDrop, [{ type: Service }], null, null);
})();
var CDK_DRAG_PARENT = new InjectionToken("CDK_DRAG_PARENT");
function assertElementNode(node, name) {
	if (node.nodeType !== 1) throw Error(`${name} must be attached to an element node. Currently attached to "${node.nodeName}".`);
}
var CDK_DRAG_HANDLE = new InjectionToken("CdkDragHandle");
var CdkDragHandle = class CdkDragHandle {
	...
	_disabled = false;
	constructor() {
		if (typeof ngDevMode === "undefined" || ngDevMode) assertElementNode(this.element.nativeElement, "cdkDragHandle");
...
var CdkDrag = class CdkDrag {
	...
	constructor() {
		...
		if (config) this._assignDefaults(config);
		if (dropContainer) {
	...
	ngAfterViewInit() {
		afterNextRender(() => {
			...
			this._dragRef.scale = this.scale;
			if (this.freeDragPosition) this._dragRef.setFreeDragPosition(this.freeDragPosition);
		}, { injector: this._injector });
	}
	ngOnChanges(changes) {
		const rootSelectorChange = changes["rootElementSelector"];
		const positionChange = changes["freeDragPosition"];
		if (rootSelectorChange && !rootSelectorChange.firstChange) this._updateRootElement();
		this._dragRef.scale = this.scale;
		if (positionChange && !positionChange.firstChange && this.freeDragPosition) this._dragRef.setFreeDragPosition(this.freeDragPosition);
	}
	ngOnDestroy() {
		if (this.dropContainer) this.dropContainer.removeItem(this);
	...
	_resetPreviewTemplate(preview) {
		if (preview === this._previewTemplate) this._previewTemplate = null;
	}
	_setPlaceholderTemplate(placeholder) {
		this._placeholderTemplate = placeholder;
	}
	_resetPlaceholderTemplate(placeholder) {
		if (placeholder === this._placeholderTemplate) this._placeholderTemplate = null;
	}
	_updateRootElement() {
		const element = this.element.nativeElement;
		let rootElement = element;
		if (this.rootElementSelector) rootElement = element.closest !== void 0 ? element.closest(this.rootElementSelector) : element.parentElement?.closest(this.rootElementSelector);
		if (rootElement && (typeof ngDevMode === "undefined" || ngDevMode)) assertElementNode(rootElement, "cdkDrag");
		this._dragRef.withRootElement(rootElement || element);
	}
	_getBoundaryElement() {
		const boundary = this.boundaryElement;
		if (!boundary) return null;
		if (typeof boundary === "string") return this.element.nativeElement.closest(boundary);
	...
	_syncInputs(ref) {
		ref.beforeStarted.subscribe(() => {
			if (!ref.isDragging()) {
				...
				if (dir) ref.withDirection(dir.value);
			}
		});
	...
	_assignDefaults(config) {
		...
		if (constrainPosition) this.constrainPosition = constrainPosition;
		if (previewClass) this.previewClass = previewClass;
		if (boundaryElement) this.boundaryElement = boundaryElement;
		if (rootElementSelector) this.rootElementSelector = rootElementSelector;
		if (previewContainer) this.previewContainer = previewContainer;
	}
	_setupHandlesListener() {
		this._handles.pipe(tap((handles) => {
			const handleElements = handles.map((handle) => handle.element);
			if (this._selfHandle && this.rootElementSelector) handleElements.push(this.element);
	...
	static ɵdir = /* @__PURE__ */ ɵɵdefineDirective({
		...
		hostBindings: function CdkDrag_HostBindings(rf, ctx) {
			if (rf & 2) ɵɵclassProp("cdk-drag-disabled", ctx.disabled)("cdk-drag-dragging", ctx._dragRef.isDragging());
		},
		inputs: {
			data: [
				0,
...
var CdkDropListGroup = class CdkDropListGroup {
...
var CdkDropList = class CdkDropList {
	element = inject(ElementRef);
	_changeDetectorRef = inject(ChangeDetectorRef);
	...
	constructor() {
		...
		if (typeof ngDevMode === "undefined" || ngDevMode) assertElementNode(this.element.nativeElement, "cdkDropList");
		...
		if (this._group) this._group._items.add(this);
	}
	addItem(item) {
		this._unsortedItems.add(item);
		item._dragRef._withDropContainer(this._dropListRef);
		if (this._dropListRef.isDragging()) this._syncItemsWithRef(this.getSortedItems().map((item) => item._dragRef));
	}
	removeItem(item) {
	...
	getSortedItems() {
		return Array.from(this._unsortedItems).sort((a, b) => {
			return a._dragRef.getVisibleElement().compareDocumentPosition(b._dragRef.getVisibleElement()) & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1;
		});
	}
	ngOnDestroy() {
		const index = CdkDropList._dropLists.indexOf(this);
		if (index > -1) CdkDropList._dropLists.splice(index, 1);
		if (this._group) this._group._items.delete(this);
	...
	_setupInputSyncSubscription(ref) {
		if (this._dir) this._dir.change.pipe(startWith(this._dir.value), takeUntil(this._destroyed)).subscribe((value) => ref.withDirection(value));
		ref.beforeStarted.subscribe(() => {
			const siblings = coerceArray(this.connectedTo).map((drop) => {
				if (typeof drop === "string") {
					const correspondingDropList = CdkDropList._dropLists.find((list) => list.id === drop);
					if (!correspondingDropList && (typeof ngDevMode === "undefined" || ngDevMode)) console.warn(`CdkDropList could not find connected drop list with id "${drop}"`);
					return correspondingDropList;
				}
				return drop;
			});
			if (this._group) this._group._items.forEach((drop) => {
				if (siblings.indexOf(drop) === -1) siblings.push(drop);
			});
			...
			if (this.elementContainerSelector) {
				...
				if (!container && (typeof ngDevMode === "undefined" || ngDevMode)) throw new Error(`CdkDropList could not find an element container matching the selector "${this.elementContainerSelector}"`);
...
var CdkDragPreview = class CdkDragPreview {
...
var CdkDragPlaceholder = class CdkDragPlaceholder {
	templateRef = inject(TemplateRef);
	_drag = inject(CDK_DRAG_PARENT, { optional: true });
	data;
	constructor() {
...
var DragDropModule = class DragDropModule {
